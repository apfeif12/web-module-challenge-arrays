...
function copy(){
    let listcopy = originalFlavors.slice();
    return listcopy
}    
...
function addFlavor(){
   let unshift = originalFlavors.unshift("Rainbow Sherbert");
...
function removeFlavorByName(array, badFlavor){
    for (let i in array) {
        if(array[i] === (badFlavor)){
            array.splice(i,1);
        return array    
        }
    }     
